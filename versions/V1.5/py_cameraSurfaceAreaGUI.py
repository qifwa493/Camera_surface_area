...
class mySettingsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.chessBoardUnitSize = self.parent._ChessBoardUnitSize
        ...
        self.settingsDialogUI.unitSizeSlider.setValue(self.chessBoardUnitSize)
        self.settingsDialogUI.unitSizeSpinBox.setValue(self.chessBoardUnitSize)
        ...
        self.settingsDialogUI.unitSizeSlider.valueChanged.connect(self.unitSizeSliderChanged)
        self.settingsDialogUI.unitSizeSpinBox.valueChanged.connect(self.unitSizeSpinChanged)
    ...
    # Change chessboard unit size set by slider bar
    def unitSizeSliderChanged(self):
        self.chessBoardUnitSize = self.settingsDialogUI.unitSizeSlider.value()
        self.settingsDialogUI.unitSizeSpinBox.setValue(self.chessBoardUnitSize)

    # Change chessboard unit size set by spin box
    def unitSizeSpinChanged(self):
        self.chessBoardUnitSize = self.settingsDialogUI.unitSizeSpinBox.value()
        self.settingsDialogUI.unitSizeSlider.setValue(self.chessBoardUnitSize)
    ...
    def accept(self):
        ...
        self.parent._ChessBoardUnitSize = self.chessBoardUnitSize
        ...
        settingsContent = json.dumps(dict(CurveFitting=self.curveFittingState,
                                          ...
                                          MaxImageSize=self.maxImgSizeState,
                                          ChessBoardUnitSize=self.chessBoardUnitSize), indent=4)
...
# This is the graphic scene for area selection in Grab Cut
...
class myWindow(QMainWindow):
    def __init__(self):
        ...
        # Load help document
        try:
            with open('app_help.txt') as f:
                helpDoc = f.read()
        except Exception as e:
            print('Error:', e)
            helpDoc = ''

        # Define about document
        aboutDoc = 'This app measures the area of a flat surface in a photograph\n\nVersion: ' + __version__ \
                   + '\n\nBy: qifwa493'
        ...
        self.MainUI.actionAbout.triggered.connect(
            lambda: QMessageBox.information(self, 'About', aboutDoc, buttons=QMessageBox.Ok))
    ...
    def openFiles(self):
        ...
        if openFilesResult == 10 or openFilesResult == 5:
            ...
            #
            # Add a soft border to the image (If you encounter error at the pre-process stage, remove 'addSoftBorder')
            self._MainImage = addSoftBorder(self._MainImage, 10)
        ...
        try:
            ...
            if self._SelectionMode == 2:
                ...
                imageWithEdges, possibleContour = findContours(self._MainImage, GrabCutMask=self._GrabCutResult,
                                                               Hull=False, MinArcLength=10, Background=False,
                                                               ChessboardDirection=self._ChessboardDirection)

            # Auto selection mode
            else:
                # Find edges and filter our 1 possible contour
                imageWithEdges, possibleContour = findContours(self._MainImage, Hull=False, MinArcLength=10,
                                                               Background=False,
                                                               ChessboardDirection=self._ChessboardDirection)
    ...
    def loadSettings(self):
        try:
            ...
            with open('app_settings.txt') as f:
                ...
                self._ChessBoardUnitSize = settingsContent['ChessBoardUnitSize']
            print('Settings loaded')

            print('Chessboard unit size:', '{:.1f}'.format(self._ChessBoardUnitSize * 0.1), 'mm')
            ...
            settingsContent = json.dumps(dict(CurveFitting=self._CurveFitting,
                                              ...
                                              ChessBoardUnitSize=self._ChessBoardUnitSize), indent=4)
    ...
    def calculateArea(self):
        ...
        try:
            ...
            if self._ContourRedraw is None:
                ...
                print('Pixels (On screen):', contourPixelArea)

            contourPixelArea *= pow(self._DrawingRatio, 2)
            print('Pixels (Actual):', contourPixelArea)
            self.MainUI.progressBar.setValue(80)
            chessboardPixelArea = getChessboardPixelArea(self._MainImage)
            # 0.1*0.1=0.01 is a factor to convert the unit of the chessboard size set in the setting dialog
            chessboardActualArea = self._ChessBoardUnitSize * self._ChessBoardUnitSize * 0.01 * 8 * 5
            ObjectArea = chessboardActualArea * (contourPixelArea / chessboardPixelArea)
            ObjectArea *= pow(10, -6)
            print('Area:', ObjectArea)
            print('')
            print('Chessboard pixels:', chessboardPixelArea)
            print('Contour pixels:', contourPixelArea)
            print('Chessboard actual:', chessboardActualArea * pow(10, -6))
            self.MainUI.progressBar.setValue(100)
            self.MainUI.statusbar.showMessage('Ready')
            # Display area
    ...
    def closeButton(self):
        # Show a dialog when the 'Exit' button is pressed
    ...
    def closeEvent(self, event):
        # Show a dialog to ask whether the user want to quit the program
        reply = QMessageBox.question(self, 'Message',
                                     "Are you sure to quit?", QMessageBox.Yes |
                                     QMessageBox.No, QMessageBox.No)
...
if __name__ == '__main__':
    ...
    # Start the main process and end the program when the main app stopped
    sys.exit(app.exec_())
