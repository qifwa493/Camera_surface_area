...
def maskChessboardAndBackground(ImageWithCB, ImageWithEdge, SampleType=2, CBD=0):
    ...
    if upperX > size[1]:
        upperX = size[1]
    mask = cv2.rectangle(mask, (lowerX, lowerY), (upperX, upperY), (255, 255, 255), -1)
    mask = cv2.rectangle(mask, (point1[0] + x, point1[1] + y), (point1[0] + x + w, point1[1] + y + h), (0, 0, 0), -1)
...
'''def maskChessboard(ImageWithCB, ImageWithEdge):
    # This function creates a mask to cover the chessboard after edge detection
    ...
    point1 = (int(x1 - 3 * dx), int(y1 - 3 * dy))
    point2 = (int(x4 + 3 * dx), int(y4 + 3 * dy))

    # Put a mask over the chessboard
    return cv2.rectangle(ImageWithEdge, point1, point2, (0, 0, 255), 5)'''


'''def maskBackground(ImageWithBackground):
    # This function crop the background of the image

    # Locate the chessboard first
    corners = getChessboardCorners(findChessboardInImage(ImageWithBackground, Width=6, Height=9))

    # Get the sample point besides the chessboard
    x1 = corners[0][0]
    y1 = corners[0][1]
    x2 = corners[1][0]
    y2 = corners[1][1]
    ...
    dx = (x4 - x1) / 8
    dy = (y4 - y1) / 5
    roi_1 = ImageWithBackground[int(y1):int(y1 + dy), int(x1 - 3 * dx):int(x1 - 2 * dx)]
    roi_2 = ImageWithBackground[int(y2 - 3 * dy):int(y2 - 2 * dy), int(x2):int(x2 + dx)]
    roi_3 = ImageWithBackground[int(y3 + 0 * dy):int(y3 + 3 * dy), int(x3):int(x3 + 3*dx)]
    roi_4 = ImageWithBackground[int(y4):int(y4 + dy), int(x4 + 3 * dx):int(x4 + 4 * dx)]

    # return histogramBackprojection(np.hstack((roi_1, roi_2)), ImageWithBackground, applyMask=False)
