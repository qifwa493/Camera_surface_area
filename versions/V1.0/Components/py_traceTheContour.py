...
def searchForNearestPoint(Image, StartingPoint, Points, Range=20):
    ...
    visited = np.vstack((Points, [StartingPoint[0], StartingPoint[1]])).copy()
...
def drawTheContour(Size, Points, Background=None, CloseLoop=False):
    ...
    for i in range(len(Points)):
        point = Points[i]
        cv2.line(black, (temp[1], temp[0]), (point[1], point[0]), (0, 0, 255), 2)
        temp = point

    if CloseLoop:
        cv2.line(black, (temp[1], temp[0]), (firstPoint[1], firstPoint[0]), (0, 0, 255), 2)
...
if __name__ == '__main__':
    fileName = 'area_sample.jpg'
    # fileName = 'complexSample.jpg'
    image = cv2.imread(fileName, 0)

    # startingPoint = (72, 102)
    startingPoint = (65, 130)
    ret, image = cv2.threshold(image, 127, 255, cv2.THRESH_BINARY)
    contour, closedLoop = traceTheContour(image, startingPoint)

    import py_getArea
    print(py_getArea.sumArea(contour))
